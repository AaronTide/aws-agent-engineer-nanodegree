#!/usr/bin/env python3
"""
cleanup_demo.py — deletes everything setup_demo.py created:
the harness, its IAM execution role, and the local harness_arn.txt.

Run:
    python cleanup_demo.py                  # default name: requirements_analyst
    python cleanup_demo.py --name my_name   # if you used a custom name
"""
import argparse
import pathlib
import time
...
def main():
    parser = argparse.ArgumentParser(description="Delete the demo harness and role")
    parser.add_argument("--name", default="requirements_analyst")
    ...
    # --- IAM role(s) ------------------------------------------------------
    # setup_demo.py gives the role a unique suffix each run, so delete
    # every role that belongs to this harness name.
